...
namespace BSPImporter
{
    ...
    public static class MeshUtils
    {
        ...
        public const float defaultScale = 0.0322f;

        // Multiple of 18 (16 + 2 borders)
        public const int lightmapAtlasSize = 1944;
        ...
        public static Vertex[] GetVerticesFromEdges(BSP bsp, Face face)
        {
            ...
            for (int i = 1; i < face.NumEdgeIndices - 1; i++)
            {
                ...
                // All tris involve first vertex, so disregard edges referencing it
        ...
        public static void CalculateUV2s(this Mesh mesh, TextureInfo textureInfo, Vector2 lightmapPos)
        {
            Vector2[] uv2 = new Vector2[mesh.vertices.Length];

            Matrix4x4 textureMatrixInverse = textureInfo.BuildTexMatrix().inverse;

            Vector3 baseTransformVertex = textureMatrixInverse.MultiplyPoint3x4(mesh.vertices[0]);

            // Origin lightmap
            Vector2 _uv0 = textureInfo.CalculateUV2(lightmapPos, new Vector2(lightmapAtlasSize, lightmapAtlasSize));
            uv2[0] = _uv0;

            Vector2 offsetRequired = Vector2.zero;

            for (int i = 1; i < uv2.Length; ++i)
            {
                Vector3 transformVertex = textureMatrixInverse.MultiplyPoint3x4(mesh.vertices[i]) - baseTransformVertex;

                transformVertex /= 16;

                // No lightmap, make it pure black
                if (lightmapPos.x == -1 && lightmapPos.y == -1) transformVertex /= 100;

                if (transformVertex.x < 0 && transformVertex.x < offsetRequired.x) offsetRequired.x = transformVertex.x;
                if (transformVertex.y < 0 && transformVertex.y < offsetRequired.y) offsetRequired.y = transformVertex.y;
                transformVertex.x += lightmapPos.x;
                transformVertex.y += lightmapPos.y;

                uv2[i] = textureInfo.CalculateUV2(transformVertex, new Vector2(lightmapAtlasSize, lightmapAtlasSize));
            }

            for (int i = 0; i < uv2.Length; ++i)
            {
                uv2[i] -= offsetRequired / new Vector2(lightmapAtlasSize, lightmapAtlasSize);
            }
        ...
        public static Mesh CombineAllMeshes(Mesh[] meshes, bool mergeSubMeshes, bool useMatrices)
        {
            ...
            // Set index format to UInt32 BEFORE combining to allow more than 65535 vertices
        ...
        /// <summary>
        /// Builds a Displacement <see cref="Mesh"/> from the <see cref="Displacement"/> referenced by
        ...
        /// <param name="face">A face referencing a <see cref="Displacement"/> and a <see cref="TextureInfo"/>.</param>
        /// <param name="dims">The dimensions of the <see cref="Texture2D"/> to map onto the resulting <see cref="Mesh"/>.</param>
        /// <returns>The <see cref="Mesh"/> created from the <see cref="Displacement"/>.</returns>
